...
interface TransactionsContextType {
  ...
  addTransaction: (
    transaction: Omit<
      Transaction,
      'id' | 'date' | 'balance' | 'index' | 'budgetId'
    > & { date: Date; budgetId?: string }
  ) => void;
  updateTransaction: (
    id: string,
    transaction: Omit<Transaction, 'id' | 'date' | 'balance' | 'index'> & {
      date: Date;
    }
  ) => void;
...
export function TransactionsProvider({ children }: { children: ReactNode }) {
  ...
  const { user } = useUser();
  const { isOffline } = usePresence();

  const transactionsQuery = useMemoFirebase(() => {
    if (!user || !firestore) return null;
    return collection(firestore, 'users', user.uid, 'transactions');
  }, [user, firestore]);
  const { data: firestoreTransactions, isLoading: transactionsLoading } =
    useCollection<TransactionFirestore>(transactionsQuery);
  ...
  useEffect(() => {
    if (isOffline) {
        toast({ title: 'Vous êtes hors ligne', description: 'Les modifications seront synchronisées une fois en ligne.' });
    } else {
    ...
    async (
      ...
      if (!firestore || !user) {
        toast({
          ...
          title: 'Erreur',
          description:
            "Impossible d'ajouter la transaction. Utilisateur non connecté.",
        });
        return;
      }
    ...
    async (
      ...
      if (!firestore || !user) {
        toast({
          ...
          title: 'Erreur',
          description:
            'Impossible de modifier la transaction. Utilisateur non connecté.',
        });
        return;
      }
    ...
    async (id: string) => {
      if (!firestore || !user) {
        toast({
          ...
          title: 'Erreur',
          description:
            'Impossible de supprimer la transaction. Utilisateur non connecté.',
        });
        return;
      }
      const docRef = doc(firestore, 'users', user.uid, 'transactions', id);
    ...
    (budget: Omit<Budget, 'id'>) => {
      if (!firestore || !user) {
        toast({
          ...
          title: 'Erreur',
          description:
            "Impossible d'ajouter le budget. Utilisateur non connecté.",
        });
        return;
      }
  ...
  const updateBudget = useCallback(
    (id: string, budget: Omit<Budget, 'id'>) => {
      if (!firestore || !user) {
        toast({
          variant: 'destructive',
          title: 'Erreur',
          description:
            'Impossible de modifier le budget. Utilisateur non connecté.',
        });
        return;
      }
    ...
    (id: string) => {
      if (!firestore || !user) {
        toast({
          ...
          title: 'Erreur',
          description:
            'Impossible de supprimer le budget. Utilisateur non connecté.',
        });
        return;
      }
      const docRef = doc(firestore, 'users', user.uid, 'budgets', id);
